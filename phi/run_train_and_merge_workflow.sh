#!/bin/bash
# 전체 워크플로우 실행 스크립트: 1. 의존성 설치 -> 2. 학습 및 병합 -> 3. 전체 모델 GGUF 변환

# 스크립트 실행 중 오류 발생 시 즉시 중단
set -e

echo "🚀 [전체 모델] 워크플로우를 시작합니다."
echo "=================================================="
...
# --- 2단계: 모델 파인튜닝 및 병합 ---
echo "✅ 2. 모델 파인튜닝 및 병합을 시작합니다 (train_and_merge_lora.py)..."
python train_and_merge_lora.py
echo "--------------------------------------------------"


# --- 3단계: 병합된 모델 GGUF 변환 ---
echo "✅ 3. 병합된 모델의 GGUF 변환을 시작합니다..."

# train_and_merge_lora.py 실행 후 out/ 디렉토리에 생성된 가장 최근 폴더를 찾습니다.
...
if [ ! -d "$MERGED_MODEL_PATH" ]; then
    ...
    echo "➡️ train_and_merge_lora.py 스크립트가 성공적으로 모델을 병합했는지 확인해주세요."
    exit 1
fi
...
chmod +x convert_merged_model_to_gguf.sh

# GGUF 변환 스크립트를 올바른 경로와 함께 실행
./convert_merged_model_to_gguf.sh "$MERGED_MODEL_PATH"
echo "--------------------------------------------------"
...
#    - run_train_and_merge_workflow.sh   (👈 이 파일)
#    - train_and_merge_lora.py           (모델 학습 및 병합 스크립트)
#    - convert_merged_model_to_gguf.sh   (병합 모델 GGUF 변환 스크립트)
#    - train.jsonl                       (학습 데이터셋)
#
# 2. 실행 방법 (Jupyter Notebook 셀):
#    # (최초 한 번만) 실행 권한 부여
#    !chmod +x run_train_and_merge_workflow.sh
#
#    # 전체 워크플로우 실행
#    !./run_train_and_merge_workflow.sh
#
