...
def load_and_prepare_jsonl(train_path: str, eval_path: Optional[str], text_field: str = "text"):
    ...
    # 허용 스키마:
    #  A) instruction + output (+ optional input)
    #  B) input + output  ← input을 instruction으로 승격
    ...
    def to_text(ex):
        ...
        # instruction이 없고 input만 있다면 -> input을 instruction으로 승격
...
def main():
    ...
    # (권장) gradient checkpointing
    try:
        model.gradient_checkpointing_enable()
    except Exception:
        pass

    # Flash Attention 2(가능 시)
    try:
