...
MODEL_PATH="/workspace/out/Phi-3.5-mini-instruct_with_lora_r128_JSON/merged_model"
...
try:
    ...
    # 타겟 위치로 복사
    target_path = "/workspace/out/Phi-3.5-mini-instruct_with_lora_r128_JSON/merged_model/tokenizer.model"
    ...
    import json
    tokenizer_json_path = "/workspace/out/Phi-3.5-mini-instruct_with_lora_r128_JSON/merged_model/tokenizer.json"

    if os.path.exists(tokenizer_json_path):
        # 빈 tokenizer.model 파일 생성 (스크립트가 파일 존재만 확인하는 경우)
        with open("/workspace/out/Phi-3.5-mini-instruct_with_lora_r128_JSON/merged_model/tokenizer.model", "w") as f:
            f.write("")
...
echo ""
echo "📦 압축 파일 생성 중..."
echo "   총 파일 크기: $(du -sh *.gguf | tail -1 | cut -f1)"
echo "   압축 진행상황을 확인하려면 다른 터미널에서 'watch ls -lh *.tar.gz*' 실행"
echo ""

# 백그라운드에서 압축 진행상황 모니터링
(
    while [ ! -f "${OUTPUT_NAME}-gguf-models.tar.gz" ]; do
        if [ -f "${OUTPUT_NAME}-gguf-models.tar.gz.tmp" ] || [ -f ".${OUTPUT_NAME}-gguf-models.tar.gz.*" ]; then
            echo "   압축 진행 중..."
        fi
        sleep 10
    done
