...
def check_training_completed(output_dir: str) -> bool:
    ...
    required_files = [
        ...
        os.path.join(merged_model_dir, "tokenizer.json"),
        # pytorch_model.bin 또는 model.safetensors 중 하나는 있어야 함
    ]
    ...
    # 모델 가중치 파일 확인 (pytorch_model.bin 또는 safetensors)
    model_weight_files = [
        os.path.join(merged_model_dir, "pytorch_model.bin"),
        os.path.join(merged_model_dir, "model.safetensors")
    ]

    # 가중치 파일 중 하나라도 있으면 OK
    weight_file_exists = any(os.path.exists(f) for f in model_weight_files)

    if weight_file_exists:
        print(f"✅ 학습이 이미 완료되어 있습니다: {merged_model_dir}")
        return True
...
def main():
    # 조건3: 학습 완료 체크
    if check_training_completed(OUTPUT_DIR):
        print("🎉 학습이 이미 완료되어 있어 건너뜁니다!")
        print(f"📁 병합된 모델 경로: {os.path.join(OUTPUT_DIR, 'merged_model')}")
        print("🔄 GGUF 변환을 진행하려면 convert 스크립트를 실행하세요.")
        return
    ...
    trainer = SFTTrainer(model=model, args=sft_config, train_dataset=train_ds, processing_class=tok,
